...
if [[ ! -e "$1" ]]; then
	echo "convert-icon: unable to open file [$1]"
	exit 1
fi

# test: imagemagick convert utility is installed
if [[ -z $(which convert) ]]; then
	echo "convert-icon: unable to find the imagemagick 'convert' utility..."
	exit 1
fi

# test: base64 encode utility is installed
if [[ -z $(which base64) ]]; then
	echo "convert-icon: unable to find the 'base64' utility..."
	exit 1
fi
...
if [[ -e "$1.rgba" ]]; then
	...
	echo "convert-icon: converted [$1] to base64 string [$1.base64]"
	rm $1.rgba
	exit 0
fi

# error: problem with imagemagick conversion
echo "convert-icon: error converting input file [$1]"
exit 1
